...
async def run_async_request(url):
...
async def get_song_id(url):
    response = await run_async_request(url)
...
async def process_multiple_page_ids(url, page_max):
    all_url_pages = [url+f"&page={page}&sort=1" for page in range(1, page_max + 1)]
    response = asyncio.gather(*[get_song_id(page) for page in all_url_pages])
    return await response

async def get_beatsaver_download_urls(song_id):
    beatsaver_url = f'https://beatsaver.com/search?q={song_id}'
    response = await run_async_request(beatsaver_url)
    soup =  BeautifulSoup(response.content, features="html.parser")
...
async def process_beatsaver_download_urls(song_ids):
    response = asyncio.gather(*[get_beatsaver_download_urls(song_id) for song_id in song_ids])
    return await response

def get_all_download_urls(song_ids):
    loop=asyncio.get_event_loop()
    page_ids = loop.run_until_complete(process_beatsaver_download_urls(song_ids))
    return list(itertools.chain(*page_ids))    


def get_all_page_ids(url, page_max):
    loop=asyncio.get_event_loop()
    page_ids = loop.run_until_complete(process_multiple_page_ids(url, page_max))
    return list(itertools.chain(*page_ids))
...
def test_songs_ids_are_retreived():
    user_id = '76561198046220364'
    scoresaber_url = f'https://scoresaber.com/u/{user_id}'
...
if __name__ == "__main__":
    ...
    # test_url = '5C870272E2159096D453F9420A8120980A5D4D29'
    # location = '/cdn/d771/5c870272e2159096d453f9420a8120980a5d4d29.zip'
    # format = /cdn/:key/:id.zip
    # beatsaver_url = 'https://beatsaver.com/search?q=' + test_url